...
///   • DiagonalVertical / CalendarVertical → near vertical + far vertical (grouped by expiry)
...
internal static class StructureOrderSplit
{
	...
	{
		if (StructureKindInfo.RequiresTwoOrders(kind))
		{
			var parsed = legs
				.Select(l => (Leg: l, Parsed: ParsingHelpers.ParseOptionSymbol(l.Symbol)))
				.Where(x => x.Parsed != null)
				.ToList();
			var groups = StructureKindInfo.SplitsByExpiry(kind)
