...
internal static class CandidateScorer
{
   /// <summary>One point in the 5-point log-normal scenario grid used to compute EV at target expiry.</summary>
	...
	{
		// Gamma weighting floors time-to-expiry at one full day, matching `wa analyze gex` (the diagnostic the
		// gravity signal is validated against). The opener's intraday TimeYearsToExpiry shrinks toward 0 through
		// a 0DTE session, which makes gamma razor-sharp at the money so gravity just tracks spot (a coincident
		// mirror) and the directional pin signal collapses. The 1-day floor keeps the gamma curve broad enough
		// that a heavy-OI strike away from spot wins gravity — the leading "magnet" behaviour — and keeps the
		// scorer's gravity identical to what `analyze gex` shows. Non-0DTE is unaffected (days ≥ 1 already).
	...
	/// </summary>
	public static decimal? ComputeSentimentFactor(decimal? sentimentScore, int directionalFit, decimal weight, bool contrarian = true)
	{
		...
		// Contrarian (default): fear (low score) → +bias → favors bullish fits, fades the crowd. Momentum:
		// invert so fear → -bias → favors bearish fits, trading WITH the regime (intraday trend / 0DTE).
		var bias = (50m - s) / 50m;
		if (!contrarian) bias = -bias;
	...
	/// <summary>Combined GEX-gravity + max-pain MAGNET expressed as a scenario-grid bias shift (in sigma
	/// units), added to <c>bias × BiasDrift</c> at every <see cref="BuildScenarioGrid"/> call. A level below
	/// spot pulls the expected-price distribution DOWN (raising long-put EV; lowering long-call and bull-credit
	/// EV) and vice versa. This is the ONLY channel that can move a structurally-negative long-premium score —
	/// the multiplicative gex/maxPain factors can't (sign-symmetric ApplyFactor caps a boost at "bring to
   ...
   /// <summary>Resolves IV from live quote → config default, as a decimal fraction (e.g. 0.40).</summary>
	...
	/// <summary>
	/// Net dealer-gamma REGIME tilt — the volatility (not directional) half of the old GEX factor. The
	/// directional gravity PIN that used to be blended in here (60%) now lives entirely in the
	/// <see cref="MagnetBiasSigmas"/> grid magnet (gexBiasPull), where it can actually move EV; this factor is
	/// purely the structure-shape tilt: negative net gamma (put-gamma dominant, amplifying regime) favors
	/// long-vol/long-premium structures, positive net gamma (suppressive) favors short-vol. Returns null when
	/// the gammaRegime weight is 0.
	/// </summary>
	private static decimal? ComputeGammaRegimeFactor(CandidateSkeleton skel, GexResult gex, OpenerConfig cfg)
	{
		if (cfg.Weights.GammaRegime <= 0m) return null;
		var envSignal = Math.Clamp(gex.NetGexFraction * VolatilityFitSign(skel.StructureKind), -1m, 1m);
		return Math.Max(0.10m, 1m + cfg.Weights.GammaRegime * envSignal);
	}
	...
	public static OpenProposal? ScoreShortVertical(CandidateSkeleton skel, decimal spot, DateTime asOf, IReadOnlyDictionary<string, OptionContractQuote> quotes, decimal bias, OpenerConfig cfg, decimal? historicalVolAnnual = null, string pricingMode = SuggestionPricing.Mid, bool applyLiquidityGate = true, decimal? sentimentScore = null, TickerEvents? events = null, IReadOnlySet<string>? snapshotTradeable = null)
	{
	   ...
	   // EV via scenario grid — payoff at expiry is piecewise linear.
		...
		var gammaRegimeFactor = ComputeGammaRegimeFactor(skel, gex, cfg);
		...
		var sentimentFactor = ComputeSentimentFactor(sentimentScore, fit, cfg.Weights.Sentiment, !string.Equals(cfg.SentimentMode, "momentum", StringComparison.OrdinalIgnoreCase));
		...
		var biasAdj = SentimentAdjust(StatArbAdjust(AssignmentRiskAdjust(GammaRegimeAdjust(VolatilityAdjust(afterFactors, netVegaPerContract, representativeIv, historicalVolAnnual, cfg.Weights.VolatilityFit), gammaRegimeFactor), assignmentFactor), statArbFactor), sentimentFactor);
	...
	private static decimal CalendarOrDiagonalPnLAtShortExpiry(decimal sT, OptionParsed shortLeg, OptionParsed longLeg, double longAtShortYears, decimal ivLong, decimal debitPerContract, ChainScalarCache? cache = null, decimal divPvAtShortExpiry = 0m)
	{
		// divPvAtShortExpiry: PV (at the short expiry) of dividends the surviving long still trades through.
		// The long's BS forward must price the coming drop; the short settles on the actual spot, unadjusted.
		var longBS = LongBlackScholes(cache, sT - divPvAtShortExpiry, longLeg.Strike, longAtShortYears, ivLong, longLeg.CallPut);
	...
	private static (decimal? lower, decimal? upper) ComputeCalendarOrDiagonalBreakevens(decimal spot, OptionParsed shortLeg, OptionParsed longLeg, double longAtShortYears, decimal ivLong, decimal debitPerContract, ChainScalarCache? cache = null, decimal divPvAtShortExpiry = 0m)
	{
		decimal Pnl(decimal s) => CalendarOrDiagonalPnLAtShortExpiry(s, shortLeg, longLeg, longAtShortYears, ivLong, debitPerContract, cache, divPvAtShortExpiry);
	...
	private static decimal FindCalendarOrDiagonalPeakPnl(decimal spot, OptionParsed shortLeg, OptionParsed longLeg, double longAtShortYears, decimal ivLong, decimal debitPerContract, ChainScalarCache? cache = null, decimal divPvAtShortExpiry = 0m)
	{
		const int steps = 240;
		var sMin = spot * 0.4m;
		var sMax = spot * 1.6m;
		...
		for (var i = 0; i <= steps; i++)
		{
			...
			var pnl = CalendarOrDiagonalPnLAtShortExpiry(s, shortLeg, longLeg, longAtShortYears, ivLong, debitPerContract, cache, divPvAtShortExpiry);
	...
	public static OpenProposal? ScoreMultiLeg(CandidateSkeleton skel, decimal spot, DateTime asOf, IReadOnlyDictionary<string, OptionContractQuote> quotes, decimal bias, OpenerConfig cfg, decimal? historicalVolAnnual = null, string pricingMode = SuggestionPricing.Mid, bool applyLiquidityGate = true, bool useMarketImpliedIv = true, decimal? sentimentScore = null, TickerEvents? events = null, IReadOnlySet<string>? snapshotTradeable = null)
	{
		...
		var biasAdj = SentimentAdjust(StatArbAdjust(AssignmentRiskAdjust(GammaRegimeAdjust(VolatilityAdjust(afterFactorsMl, netVegaPerContract, representativeIv, historicalVolAnnual, cfg.Weights.VolatilityFit), gammaRegimeFactor), assignmentFactor), statArbFactor), sentimentFactor);
	...
	public static OpenProposal? ScoreCalendarOrDiagonal(CandidateSkeleton skel, decimal spot, DateTime asOf, IReadOnlyDictionary<string, OptionContractQuote> quotes, decimal bias, OpenerConfig cfg, decimal? historicalVolAnnual = null, string pricingMode = SuggestionPricing.Mid, bool applyLiquidityGate = true, bool useMarketImpliedIv = true, decimal? sentimentScore = null, TickerEvents? events = null, IReadOnlySet<string>? snapshotTradeable = null)
	{
		...
		// Dividend handling, two windows. (1) divPvAtShortExpiry: PV at the SHORT expiry of dividends the
		// surviving long still trades through — a spot-independent forward shift folded into every terminal
		// valuation (BE/POP/EV/maxProfit) below. Dividends on/before the short expiry are excluded: by then
		// the spot path has already absorbed the drop, and both legs crossed it symmetrically. (2) the long's
		// IV back-solve must invert its mid at TODAY's dividend-adjusted spot (the mid prices the coming
		// drop), or the dividend gets double-counted as vol. Provider ex-dates are holiday-snapped upstream
		// in DividendScheduleBuilder.
		...
		var divPvAtShortExpiry = spot - OptionMath.DividendAdjustedSpot(spot, dividends, shortExpiryClose, longExpiryClose, OptionMath.RiskFreeRate);
		...
		var (beLower, beUpper) = ComputeCalendarOrDiagonalBreakevens(spot, shortParsed, longParsed, longAtShortYears, ivLong, debitPerContract, cache, divPvAtShortExpiry);
		decimal pop;
		...
		else if (beLower.HasValue)
		{
			...
			var profitableAbove = CalendarOrDiagonalPnLAtShortExpiry(beLower.Value * 1.02m, shortParsed, longParsed, longAtShortYears, ivLong, debitPerContract, cache, divPvAtShortExpiry) > 0m;
			pop = profitableAbove
		...
		decimal PnlAtTarget(decimal sT) => CalendarOrDiagonalPnLAtShortExpiry(sT, shortParsed, longParsed, longAtShortYears, ivLong, debitPerContract, cache, divPvAtShortExpiry);
		decimal ev = 0m;
		foreach (var pt in grid)
			ev += pt.Weight * PnlAtTarget(pt.SpotAtExpiry);
		var maxProfit = FindCalendarOrDiagonalPeakPnl(spot, shortParsed, longParsed, longAtShortYears, ivLong, debitPerContract, cache, divPvAtShortExpiry);
		...
		var biasAdj = SentimentAdjust(StatArbAdjust(AssignmentRiskAdjust(GammaRegimeAdjust(VolatilityAdjust(afterFactorsCd, netVegaPerContract, representativeIv, historicalVolAnnual, cfg.Weights.VolatilityFit), gammaRegimeFactor), assignmentFactor), statArbFactor), sentimentFactor);
		var finalScore = ComputeFinalScore(biasAdj, thetaPerDayPerContract, efficiencyCapital);
		var fp = ComputeFingerprint(skel.Ticker, skel.StructureKind, skel.Legs, qty: 1);
	...
	public static OpenProposal? ScoreLongCallPut(CandidateSkeleton skel, decimal spot, DateTime asOf, IReadOnlyDictionary<string, OptionContractQuote> quotes, decimal bias, OpenerConfig cfg, decimal? historicalVolAnnual = null, string pricingMode = SuggestionPricing.Mid, bool applyLiquidityGate = true, decimal? sentimentScore = null, TickerEvents? events = null, IReadOnlySet<string>? snapshotTradeable = null)
	{
		...
		// gex (gravity) is needed below for the multiplicative factor; it also feeds the grid magnet via
		// MagnetBiasSigmas — a gravity/max-pain level below spot pulls the EV distribution down so a long put's
		...
		var maxPain = MaxPainCached(skel.Ticker, skel.TargetExpiry.Date, quotes, spot);
		// gex (gravity) was computed above for the scenario-grid magnet; reuse it for the multiplicative factor.
		var gammaRegimeFactor = ComputeGammaRegimeFactor(skel, gex, cfg);
		var assignmentFactor = ComputeAssignmentRiskFactor(skel, spot, asOf, ResolveStrikeStep(cfg, skel.Ticker), bias);
		...
		var biasAdj = SentimentAdjust(StatArbAdjust(AssignmentRiskAdjust(GammaRegimeAdjust(VolatilityAdjust(afterFactorsLc, netVegaPerContract, iv, historicalVolAnnual, cfg.Weights.VolatilityFit), gammaRegimeFactor), assignmentFactor), statArbFactor), sentimentFactor);
		var finalScore = ComputeFinalScore(biasAdj, thetaPerDayPerContract, capitalAtRisk);
		var fp = ComputeFingerprint(skel.Ticker, skel.StructureKind, skel.Legs, qty: 1);
