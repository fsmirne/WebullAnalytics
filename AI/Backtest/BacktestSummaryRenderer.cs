...
internal static class BacktestSummaryRenderer
{
	public static void Render(BacktestResult result, bool showFills, bool bookCmd = false)
	{
		...
		AnsiConsole.MarkupLine("[dim]  • Price foundation = REAL minute NBBO from the ThetaData quote store (data/quotes.db): each leg is marked at its real bid/ask/mid and the configured fill model crosses from there (`--pricing mid` = real mid + slippagePerSharePerOrder; `--pricing bidask` crosses the full real spread). Legs with no quote within the staleness window fall back to a Black-Scholes parametric source (SPX-family uses VIX as ATM IV, other tickers 30-day realized HV × premium) with a per-ticker synthetic half-spread; that same parametric source also answers counterfactual intraday reprices (SL/TP brackets, the profit projector) that no historical NBBO can. The provenance line below (real-priced vs synthetic) is the fraction of fill legs backed by the real NBBO store.[/]");
	...
	{
		null or "" => "—",
		"CloseBeforeShortExpiryRule" => "CloseAtExp",
