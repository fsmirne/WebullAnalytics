...
internal sealed class RuleEvaluator
{
	...
	private static ManagementProposal AttachDiagnostic(ManagementProposal proposal, OpenPosition position, EvaluationContext ctx)
	{
		...
		var historicalVolAnnual = ctx.HistoricalVolByTicker != null && ctx.HistoricalVolByTicker.TryGetValue(position.Ticker, out var hv) ? hv : (decimal?)null;
		var diagnostic = RiskDiagnosticBuilder.Build(diagLegs, spot, ctx.Now, IvResolver, trend: null, quotes: ctx.Quotes);
		var probe = RiskDiagnosticProbeBuilder.Build(diagLegs, spot, ctx.Now, IvResolver, ctx.Quotes, opener: null, technicalBiasOverride: technicalBias, historicalVolAnnual: historicalVolAnnual);
